...
| (2) Business-readable output | **v2 partial** | Scenarios whose steps are all in-process-eligible (currently: `cross-margin-survival`, `cross-margin-fail`) take the v2 path: in-process dispatch into `run_lending_demo_structured`, then HEADLINE / TIMELINE / DELTA / OUTCOMES / NEXT contract per `fabrknt/website/SANDBOX-PATTERN.md`. Scenarios with sub-process steps (currently: `manual-lending-walkthrough`) keep the v1 sub-process passthrough. OUTCOMES still emits a placeholder; Phase 3 will parse `expected_outcomes` from JSON and verify each ✓/✗. |
