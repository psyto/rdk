...
1. **Bad-debt Layer 3 (L-5)** — Layers 1 (algorithmic halt) and 2 (operator agreement, see [`operator-agreement.md`](./operator-agreement.md)) are in place per [ADR-010](./adr/010-bad-debt-depletion-policy.md); Layer 3 (`socialize_residual` impl + admission check + `SocializationEvent` chain-history append) needs code. Required before v1 mainnet.
