...
| L-5 | Bad debt accumulation: cascade of underwater positions exceeds insurance fund | Sustained market crash | 🟠 | ✅/🚧 **Layer 1 (algorithmic halt) ✅**: `PrincepsNode::absorb_lending_bad_debt` routes shortfall to InsuranceFund; `LendingHaltParams` (v0 default 50% running-coverage threshold / 200-block halt / 128-block window) arms `lending_halt_until` at end-of-tick when the fund balance can no longer cover a halt window's typical shortfall; bin/princeps skips the unified scan + bridge-side bad-debt absorption loop while halted. Halt persists across restart via `CoordinatorSnapshot::lending_halt_until`. Repays + liquidations + healthy-position withdraws remain allowed — they reduce risk. **Layer 2 (operator-cap commitment + 72-hour disclosure) ✅ template**: codified in [`operator-agreement.md`](./operator-agreement.md) §4 + §5; template in place, signed instances awaited at v1 mainnet onboarding (no validator credentials issued at v0 testnet). **Layer 3 (manual-declaration `socialize_residual`) deferred to v1 multi-asset / `scaled_supply` work** — at v0 the lending supply side is the bridge-owned pool (`lending::accrual.rs` is explicit on this); there is no third-party depositor population to socialize against, so Layer 3 has no current target. Gated on three pieces not built at v0: per-depositor `scaled_supply` on `lending::Position`, princeps-side chain history (port openhl Stage 21 pattern), and operator-sig admission infrastructure. See [ADR-010 Decision > Layer 3 deferral note](./adr/010-bad-debt-depletion-policy.md). v0 scope limit: precompile-level borrow-during-halt enforcement is v1 work (same constraint as oracle CB row O-3). | [ADR-010](./adr/010-bad-debt-depletion-policy.md), [`operator-agreement.md`](./operator-agreement.md), `princeps/crates/node/src/lib.rs::evaluate_lending_halt`, `crates/liquidation/src/insurance.rs` |
...
1. **Bad-debt Layer 3 (L-5) — deferred to v1 multi-asset / `scaled_supply` work** — Layers 1 (algorithmic halt) and 2 (operator agreement, see [`operator-agreement.md`](./operator-agreement.md)) are in place per [ADR-010](./adr/010-bad-debt-depletion-policy.md). Layer 3 (`socialize_residual` + admission check + `SocializationEvent` chain-history append) has no current target at v0 (bridge-owned pool, no third-party depositors); implementation is gated on three pieces not built at v0 (`scaled_supply` on `lending::Position`, princeps-side chain history, operator-sig admission). Must land before v1 multi-asset markets onboard real depositors. See [ADR-010 Decision > Layer 3 deferral note](./adr/010-bad-debt-depletion-policy.md).
...
The previously-listed "ETH oracle deviation circuit breaker (O-3)" gap was closed in this commit cycle — see the O-3 row above for the new ✅ status. The L-5 "operator policy on InsuranceFund depletion" gap was partially closed by ADR-010 Layer 1 (algorithmic halt) and Layer 2 (operator agreement; see [`operator-agreement.md`](./operator-agreement.md)). Layer 3 (manual-declaration socialization) was specified against infrastructure that doesn't exist at v0 (per-depositor `scaled_supply`, princeps-side chain history, operator-sig admission) and was deferred to the v1 multi-asset / `scaled_supply` work — at v0 there is no third-party depositor population to socialize against (the supply side is the bridge-owned pool). See [ADR-010 Decision > Layer 3 deferral note](./adr/010-bad-debt-depletion-policy.md) and the [L-5 row](#lending-economic-attacks) above.
