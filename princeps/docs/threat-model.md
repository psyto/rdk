...
1. **Bad-debt depletion policy beyond Layer 1 (L-5)** — Layer 1 (algorithmic lending halt) landed per [ADR-010](./adr/010-bad-debt-depletion-policy.md). Layer 2 (ADR-008 operator-agreement amendment with make-whole + 72-hour-disclosure clauses) is doc work; Layer 3 (`socialize_residual` impl + admission check + `SocializationEvent` chain-history append) needs code. Both required before v1 mainnet.
...
The previously-listed "ETH oracle deviation circuit breaker (O-3)" gap was closed in this commit cycle — see the O-3 row above for the new ✅ status. The L-5 "operator policy on InsuranceFund depletion" gap was partially closed by ADR-010 Layer 1 (algorithmic halt; see L-5 row above for the new ✅/🚧 status) — Layers 2 and 3 remain.
