...
| E-4 | Precompile gas pricing makes a useful call uneconomic, blocking liquidations | Reth gas schedule | 🟡 | ✅/🚧 **Differentiated per-precompile costs** in `princeps/crates/evm/src/precompiles/mod.rs` (the original uniform `LENDING_BASE_GAS_COST = 2_000` was the original Gap referenced here). 9 constants now reflect computational class: HEALTH 1_500 (read-only) → DEPOSIT 2_000 → REPAY/SUPPLY 2_500 → WITHDRAW_SUPPLY 3_000 → BORROW/WITHDRAW_COLL 4_000 → LIQUIDATE 6_000 → SOCIALIZE 8_000 (ECDSA verify + market mutation + chain-history append). Each handler uses the same constant for success AND zero-error paths so gas pricing doesn't leak validation outcome. Real criterion benchmarks against a reference workload remain pre-mainnet work — current numbers are educated-guess starting points loosely calibrated against Ethereum-precompile reference values. | `princeps/crates/evm/src/precompiles/mod.rs` (the per-precompile `LENDING_*_GAS_COST` constants), [v0 plan](./plans/v0-lending.md#open-questions--risks) |
...
2. **Precompile gas pricing criterion benchmarks (E-4)** — per-precompile constants landed (9 differentiated values matching computational class). Real criterion benchmarks against a reference workload remain for pre-mainnet promotion — the current numbers are educated-guess starting points loosely calibrated against Ethereum-precompile reference values.
